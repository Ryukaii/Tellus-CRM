...
export function LeadForm() {
  ...
  const validateCPFAndCheckExisting = async () => {
    ...
    try {
      const result = await CPFValidationService.validateCPF(cleanCPF, 'geral');
      return result;
    } catch (error) {
  ...
  const handleAddProcess = async () => {
    ...
    try {
      ...
      await CPFValidationService.addProcessToExistingCustomer(cleanCPF, 'geral');
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-tellus-gold-100">Crédito Imobiliário</p>
                </div>
              </div>
              {sessionId && (
